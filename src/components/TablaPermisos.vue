...
export default {
  ...
  methods: {
    ...
    async createPermiso(permiso) {
      try {
        const response = await PermisoDataService.create(permiso);
        permiso.id = response.data.data.id;
    ...
    async updatePermiso(id, permiso) {
      try {
        await PermisoDataService.update(id, permiso);
      } catch (err) {
        // Handle Error Here
        console.error(err);
      }
    ...
    async destroy(id) {
      try {
        await PermisoDataService.delete(id);
      } catch (err) {
        // Handle Error Here
        console.error(err);
      }
    ...
    save() {
      if (this.editedIndex > -1) {
        this.updatePermiso(this.editedItem.id, this.editedItem);
        Object.assign(this.permisos[this.editedIndex], this.editedItem);
      } else {
        this.createPermiso(this.editedItem);
      }
      this.close();
    },
  },
